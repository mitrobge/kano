...
class Surveys 
{
    ...
    public static function GetSurveyIsActive($surveyId, $languageId = null)
    {
        // Get current session's languageId if not specified
        if (is_null($languageId))
            $languageId = Language::Get();
		...
		// Build SQL query
		$sql = 'CALL surveys_get_survey_isactive(:survey_id, :language_id)';
    ...
    public static function UpdateSurveyQuestion($questionId, $questionName, $isPositive, $languageId)
    {
	// Build SQL query
        $sql = 'CALL surveys_update_survey_question(:question_id, 
            :question_name, :is_positive, :language_id)';
